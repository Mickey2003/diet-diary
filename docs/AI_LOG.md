...
| 09-15 | v0.8.6 菜品配图 | 取图降级链（缓存 → AI 生图 → 图库 → emoji）、SenseAudio 生图客户端、emoji 四级映射、WebP + 200×150 缩略图、一键批量替换 | 指定用商汤 SenseAudio 并给出风格要求；实机点开关发现「启用 AI 生图」刷新后复位，反馈后修复（案例 19） |
...
### 案例 19：「启用 AI 生图」开关点了没用，刷新就打回关闭（v0.8.6 修复）
- **现象**：用户在「模型设置 → 菜品配图」把「启用 AI 生图」拨到开启，刷新页面后又变回关闭，像是开关失效。
- **AI 的错误**：写这个开关时只接了 `onChange={setDishAiImages}`（改本地 state），把落盘交给了**另一个卡片**里「保存设置」按钮的 `handleSave()`。但那个按钮在页面下方老远，用户根本不会想到要先拨开关、再滚下去点保存 —— 开关在交互直觉上就应该**拨完即生效**。同一张卡片后来虽然加了「保存生图 API」按钮，但它也没带 `dish_ai_images` 字段。
- **定位**：先直连服务端做往返验证 —— `PUT /api/settings {"dish_ai_images": true}` 后再 `GET`，值确实是 `true`，说明**后端没问题**；再看前端，`grep dish_ai_images` 只出现在 `handleSave()` 里，而开关的 `onChange` 只调 `setDishAiImages`。于是确认是"改了本地状态但从未发请求"。
- **修复**：新增 `handleToggleAiImages()`，切换后**立即调用 `updateSettings`**，采用**乐观更新**（先改界面保证手感跟手）+ **失败回滚**；顺带给开关加 `loading`，并在"已开启但生图 API 没配全"时给出提示而不是静默失败。另加两条回归测试 `test_dish_ai_images_switch_persists_alone` / `test_ai_images_enabled_reads_switch`，锁住"单独传这一个字段也能落盘、且刷新后仍读到"。
- **学到**：
  1. **"能保存"不等于"用户能保存"**：把开关和保存按钮放在不同卡片里，在实现上没问题、在体验上就是 bug。凡是**开关 / 单选框 / 分段控件**这类即时型控件，都应即时落盘，不要依赖远处的提交按钮。
  2. 排查"改完就丢"类问题，**先用一次 API 往返判定责任在前后端哪一侧**，比两头读代码快得多。
  3. 乐观更新必须配**失败回滚**，否则界面会显示一个服务端并不存在的状态。
